...
def embedGifs(pathString):
    equation = Image.open(r"Share\\BesselTall.png")
    path   =  Image.open(r"paths\\"+pathString+r"Path.gif")
    pattern =  Image.open(r"C:\Users\Michael\Documents\Programming\NewtonBasinImages\Bessel\variableOffset\\"+str(pathString)+"\\GIF.gif")
    ...
    for i in range(90):
        ...
        frame.paste(equation, (25,475) , mask=equation.convert("RGBA"))
        frames.append(frame)
...
plt.tight_layout()
ani.save(r"paths/LissaJous_A2B3D3pi4Path.gif",writer=writergif)
plt.show()
plt.clf()
embedGifs("LissaJous_A2B3D3pi4")